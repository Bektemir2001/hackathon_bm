...
class OnlySubcategory extends StatefulWidget {
  final id;
  final subCategoryName;
  OnlySubcategory({Key? key, required this.id,required this.subCategoryName}) : super(key: key);


  @override
  _OnlySubcategoryState createState() => _OnlySubcategoryState();
}

class _OnlySubcategoryState extends State<OnlySubcategory> {
  ...
  void initState() {
    super.initState();
    futureProductSubcategory = fetchProductSubcategory(widget.id);
  }
    ...
    return Scaffold(
      ...
      body: ListView(
          ...
          children: [
            ...
            FutureBuilder<ProductSubcategoryModel>(
              future: futureProductSubcategory,
                    ...
                    Wrap(
                      ...
                      children: [
                        for (var i=0;i<items;i++)
                          SlideContainer(short[i].image.toString(),short[i].name.toString(),short[i].description.toString(),short[i].price!,short[i].count!,short[i].colors!,short[i].id!.toString())
                      ],
                    );
  ...
  Widget SlideContainer(
      String image, String name,String description,int price,int count,var colors,String id) {
    return GestureDetector(
      onTap: ()=>Navigator.of(context).push(MaterialPageRoute(builder:(context)=>Product2(name: name,description: description,price: price,count: count,mainImage: image,colors: colors,id: id,))),
      child: Container(
        child: Row(
          children: [
            Container(
